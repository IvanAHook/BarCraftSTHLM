...
class sthlmesportThemeHooks implements Gdn_IPlugin {
   ...
   public function OnDisable() {
	...
	public function SettingsController_AfterCurrentTheme_Handler($Sender) {
		$SingleColumn = C('Themes.sthlmesport.SingleColumn');
		echo Wrap(
			...
			.Wrap(Anchor(
				...
				'settings/sthlmesporttogglepanel/'.Gdn::Session()->TransientKey(),
				'SmallButton'
			), 'div')
		, 'div', array('class' => 'Description'));
	}
	
	public function SettingsController_sthlmesportTogglePanel_Create($Sender) {
	...
	public function PluginController_BeforeEmbedRecommend_Handler($Sender) {
		...
		echo '<div class="EmbedRecommend">
			...
			.Wrap(
				...
				.Wrap(Anchor(
					T($SingleColumn ? 'Show the side panel' : 'Hide the side panel'),
					'plugin/sthlmesporttogglepanel/'.Gdn::Session()->TransientKey(),
					'SmallButton'
	...
	public function PluginController_sthlmesportTogglePanel_Create($Sender) {
		$this->_TogglePanel($Sender);
	...
	private function _TogglePanel($Sender) {
		...
		if (Gdn::Session()->ValidateTransientKey($TransientKey))
			SaveToConfig('Themes.sthlmesport.SingleColumn', C('Themes.sthlmesport.SingleColumn') ? FALSE : TRUE);
	}


	public function Base_Render_Before($Sender) {
		if (($Sender->MasterView == 'default' || $Sender->MasterView == '') && C('Themes.sthlmesport.SingleColumn'))
   ...
   public function DraftsController_Render_Before($Sender) {
	...
	private function _AddButton($Sender, $ModuleName) {
		if (C('Themes.sthlmesport.SingleColumn'))
